...
`install_tables STR_VAR table dir destination_dir = "override" tra = "*" patches = "*"`

Installer for copying tables (`.2da` files) to `destination_dir` (defaulting to `override`) with information from `table` (full path) and files located in `dir`. `patches` is an optional file to be `INCLUDED` containing patching functions referenced by name in `table` and `tra` is an optional tra file (full path) to load. `table` must have the format of [tables template table](../../../resources/2da/installers/templates/tables.2da)
